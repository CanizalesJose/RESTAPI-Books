...
class loansDAO{
    ...
    static async fetchPending() {
        const sqlQuery = 'SELECT Loans.id as loanId, loanDetails.bookId as bookId, loanDetails.returned as returned, Users.username as username, Users.contactNumber as contactNumber, Users.email as email, DATE_FORMAT(loans.loanDate, "%d-%m-%Y") as date, DATE_FORMAT(loans.returnDate, "%d-%m-%Y") as returnDate, Books.title as title, Books.isbn as isbn, Books.imageUrl as cover, Authors.fullName as author, Categories.descr as category FROM Loans INNER JOIN loanDetails ON Loans.id = loanDetails.loanId INNER JOIN Books ON loanDetails.bookId = Books.id INNER JOIN Authors ON Books.author = Authors.id INNER JOIN Categories ON Books.category = Categories.id INNER JOIN Users ON loans.username = Users.username WHERE returned = 0 ORDER BY returnDate DESC';
        return db.query(sqlQuery)
        .then(res => {
            return res;
        })
